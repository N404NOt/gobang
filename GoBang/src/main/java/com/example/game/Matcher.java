    ...
    private void handlerMatch(Queue<User> matchQueue) {
        synchronized (matchQueue) {
            try {
                ...
                // 3.获取到玩家的 websocket 的绘画
                // 获取到会话的目的是为了告诉玩家，排到了对手
                ...
                if(session2 == null) {
                    // 如果玩家2 现在下线了。把玩家1 放回匹配队列中
                    matchQueue.offer(player1);
                    return;
                }
                // 当前能否排到两个玩家是同一个玩家的情况么？一个玩家入队两次
                // 理论上应该不会
                // 1) 如果玩家下线，就对玩家移除匹配队列
                // 2) 禁止玩家多开
                // 但是以免前面逻辑出现bug带来的后果再进行一次判断
