...
public class GameAPI extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        // 3.判定当前是不是多开 (该用户是否在其他地方进入游戏了)
        if(onlineUserManager.getFromGameHall(user.getUserId()) != null
                || onlineUserManager.getFromGameRoom(user.getUserId()) != null) {
            // 如果一个账号，一边在游戏大厅，一边在游戏房间，也认为是多开
            resp.setOk(true);
            resp.setReason("禁止多开游戏界面");
            resp.setMessage("respeatConnection");
            session.sendMessage(new TextMessage(objectMapper.writeValueAsString(resp)));
        }
        ...
        synchronized (room) {
            ...
            if (room.getUser2() == null) {
                // 来到这里说明玩家1 已经加入房间,现在要给当前玩家作为玩家2 加入房间
                room.setUser2(user);
                System.out.println("玩家: " + user.getUsername() + " 已经准备就绪！作为玩家2!");

                // 当两个玩家都加入成功后，让服务器给这两个玩家都返回websocket的信息
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        ...
        if(session == exitSession) {
            // 加上这个判定，目的是为了避免在多开的情况下，第二个用户退出连接动作，导致第一个用户异常移除
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        System.out.println("当前用户： " + user.getUsername()+" 已经离开房间!");
    }
}
